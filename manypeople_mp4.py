...
# Initialize video capture for the mp4 file
...
frame_number = 0
results = []
min_face_area = 1000  # 얼굴의 최소 면적(너무 작은 얼굴, 전신 샷 제외)
...
def calculate_TW(eyebrow_angle, chin_angle, philtrum_length):
    if len(eyebrows) > 1 and len(chin_angles) > 1 and len(philtrums) > 1:
        ...
        chin_angle_std = (chin_angle - np.mean(chin_angles)) / np.std(chin_angles)
...
# 엑셀 파일로 저장
with pd.ExcelWriter(r"C:\Users\세은\OneDrive\바탕 화면\fwhr_tw_analysis_with_auto_face_id.xlsx") as writer:
    df.to_excel(writer, sheet_name="Frame_Data", index=False)
    summary_stats.to_excel(writer, sheet_name="Summary_Statistics", index=False)

print(f"Results and summary saved successfully to {writer}")
